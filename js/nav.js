...
const NAV_ITEMS = [
  ...
  { href: "crimes.html", label: "Crimes", icon: "crimes" },
  { href: "gym.html", label: "Gym", icon: "gym" },
  { href: "jobs.html", label: "Jobs", icon: "jobs" },
  { href: "bank.html", label: "Bank", icon: "bank" },
  { href: "travel.html", label: "Travel", icon: "travel" },
  { href: "gang.html", label: "Gang", icon: "gang" },
  { href: "attack.html", label: "Attack", icon: "attack" },
...
export function renderShell() {
  ...
  const links = NAV_ITEMS.map(
    (item) =>
      `<a href="${item.href}" class="${item.href === current ? "active" : ""}">${icon(item.icon)}<span>${item.label}</span></a>`
  ).join("");
  ...
  document.body.innerHTML = `
    <div class="app-shell">
      <nav class="rail">
        ...
        ${links}
        <a href="#" id="logout-link" class="logout-link">${icon("logout")}<span>Log out</span></a>
      </nav>
      <main class="main">
...
export function renderRapSheet(user, maxes) {
  ...
  if (heroEl) {
    ...
    heroEl.innerHTML = `
      <div class="hero-banner">
        ${citySkylineSvg(cityId, { height: 140 })}
        <div class="hero-caption">${user.city}</div>
        <div class="hero-tag">LVL ${user.level} · ${user.username}</div>
      </div>
    `;
  }
  ...
  el.innerHTML = `
    <div class="rap-cell">
      <div class="label">${icon("cash", { size: 12 })} Cash</div>
      <div class="value brass">$${Math.floor(user.money).toLocaleString()}</div>
    </div>
    <div class="rap-cell">
      <div class="label">${icon("vault", { size: 12 })} Bank</div>
      <div class="value">$${Math.floor(user.bank).toLocaleString()}</div>
    </div>
    <div class="rap-cell">
      <div class="label">${icon("heart", { size: 12 })} Health ${Math.floor(user.health)}/${maxes.maxHealth}</div>
      ${bar(user.health, maxes.maxHealth, "health")}
    </div>
    <div class="rap-cell">
      <div class="label">${icon("bolt", { size: 12 })} Energy ${Math.floor(user.energy)}/${maxes.maxEnergy}</div>
      ${bar(user.energy, maxes.maxEnergy)}
    </div>
    <div class="rap-cell">
      <div class="label">${icon("brave", { size: 12 })} Brave ${Math.floor(user.brave)}/${maxes.maxBrave}</div>
      ${bar(user.brave, maxes.maxBrave)}
    </div>
    <div class="rap-cell">
      <div class="label">${icon("will", { size: 12 })} Will ${Math.floor(user.will)}/${maxes.maxWill}</div>
      ${bar(user.will, maxes.maxWill)}
    </div>
    <div class="rap-cell">
      <div class="label">${icon("pin", { size: 12 })} Location</div>
...
function addAdminLinkIfMissing() {
  ...
  link.innerHTML = `${icon("admin")}<span>Admin</span>`;
